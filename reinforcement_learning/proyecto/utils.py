...
def generate_video_with_qvalues(agent, output_filename='agent_maze.gif', fps=2, max_steps=100):
    ...
    print(f"Trayectoria: {len(trajectory)} estados, {len(actions_taken)} acciones")
    ...
    def draw_maze_base(ax):
        ...
        # Celda meta (verde)
        goal_row, goal_col = agent.env.goal_state
        ax.add_patch(patches.Rectangle((goal_col, goal_row), 1, 1, 
                                       facecolor='#68FF33', alpha=0.5, edgecolor='black', linewidth=2))
    ...
    def draw_path(ax, current_step):
        ...
        if current_step > 0:
            ...
            ax.plot(cols, rows, 'b--', linewidth=2, alpha=0.5, label='Trayectoria')
